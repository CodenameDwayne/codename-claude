...
const VALID_TYPES = new Set([
  'agent:active', 'agent:idle', 'handoff', 'verdict',
  'task:progress', 'pipeline:start', 'pipeline:end',
  'heartbeat', 'state:snapshot',
]);

export function isValidWSEvent(event: WSEvent): boolean {
  return VALID_TYPES.has(event.type);
}
